...
function applyOverride(data, { gw, payers, reason }, now, who = 'treasurer') {
  ...
  data.managers.forEach(m => {
    ...
    if (has && !should) {
      ...
      if (f) { f.reversed = true; f.reversed_reason = `GW${gw} bottom-3 override: ${reason}`; f.reversed_by = who; f.reversed_date = now; }
      ...
      m.fines.push({
        ...
        added_by: `treasurer-override:${who}`, added_date: now,
        edited_by: null, edited_date: null, edit_history: [],
      });
    }
  });
  (data.overrides = data.overrides || []).push({ gw, payers: [...want], reason: String(reason).trim(), by: who, date: now });
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    await mutateData(`Treasurer (${who}): override GW${gw} bottom-3 result`, (data) => {
      applyOverride(data, { gw, payers, reason }, now, who);
      return true;
    });
