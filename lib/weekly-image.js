...
// is loaded via dynamic import (see lib/satori-render.js, shared with brag-image.js).
// Fonts + group photo are bundled repo assets (see vercel.json includeFiles) and
// cached at module scope there.
...
function card(d) {
  const HH = 500;
  // The card's canvas height was fixed, tuned for exactly 3 in "The Drop" —
  // but the league rule pays everyone at or below the 3rd-lowest score, so a
  // tie can make that 4, 5, etc. Extra rows then had nowhere to go and ran
  // straight into the footer text. Grow the canvas per row above 3 instead.
  const height = 1560 + Math.max(0, d.drop.length - 3) * 100;
...
async function renderWeeklyPng(data) {
  const height = 1560 + Math.max(0, data.drop.length - 3) * 100;
