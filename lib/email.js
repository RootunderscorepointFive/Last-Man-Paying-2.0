...
function outstandingOf(manager, joiningFee = 300) {
...
async function resendSend({ to, subject, html, text, attachments }) {
  ...
  // Resend caps at ~10 requests/second. On a 429, honour Retry-After (falling
  // back to ~1.1s) and retry, so a rate-limited send isn't silently dropped.
  for (let attempt = 0; ; attempt++) {
    ...
    if (r.status === 429 && attempt < 4) {
      ...
      await new Promise(res => setTimeout(res, ra > 0 ? ra * 1000 : 1100));
      continue;
    }
...
// Headline numbers as text (survives image-blocking); the full infographic rides
// along as an attachment. `w` is the object from lib/weekly.js buildWeekly().
function weeklySummaryEmail(w) {
  const R = n => 'R' + Number(n || 0).toLocaleString();
  const line = (label, value, color) => `<tr><td style="padding:5px 0;color:${C.muted};">${esc(label)}</td><td align="right" style="padding:5px 0;color:${color || C.text};font-weight:bold;">${value}</td></tr>`;
  const dropRows = w.drop.map((p, i) => `<tr><td style="padding:6px 0;color:${C.red};font-weight:bold;width:24px;">${i + 1}</td><td style="padding:6px 0;color:${C.text};">${esc(p.name)} <span style="color:${C.muted};">· ${esc(p.team)}</span></td><td align="right" style="padding:6px 0;color:${C.red};font-weight:bold;">${p.pts} pts</td></tr>`).join('');
  const tableRows = w.standings.map((m, i) => `<tr><td style="padding:4px 0;color:${C.amber};font-weight:bold;width:24px;">${i + 1}</td><td style="padding:4px 0;color:${C.text};">${esc(m.name)}</td><td align="right" style="padding:4px 0;color:${C.green};">${m.pts} pts</td></tr>`).join('');
  const mv = w.mover ? `${esc(w.mover.name)} · ${w.mover.delta >= 0 ? '+' : '−'}${Math.abs(w.mover.delta)}` : '—';
  const inner = `
    <div style="font-size:20px;font-weight:bold;color:${C.amber};">GW${w.gw} · Weekly Report</div>
    <div style="color:${C.muted};font-size:12px;margin-top:2px;">Season ${esc(w.season || '')} · the full infographic is attached — share it in the group.</div>
    ${section('This Week')}
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">
      ${line('Top scorer', w.top ? `${esc(w.top.name)} · ${w.top.pts} pts` : '—', C.green)}
      ${line('Captain of the week', w.captain ? `${esc(w.captain.name)} · ${w.captain.count}×` : '—', C.amber)}
      ${line('Biggest mover', mv, C.text)}
      ${line('Fines this GW', R(w.finesThisGw), C.red)}
      ${line('Prize pool', R(w.poolLevied), C.text)}
    </table>
    ${section('The Drop · Bottom ' + w.drop.length + ' · R100 each')}
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">${dropRows}</table>
    ${section('Top of the Table')}
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">${tableRows}</table>`;
  const text = [
    `GW${w.gw} · Weekly Report — Season ${w.season || ''}`,
    ``,
    `Top scorer: ${w.top ? `${w.top.name} (${w.top.pts})` : '—'}`,
    `Captain of the week: ${w.captain ? `${w.captain.name} x${w.captain.count}` : '—'}`,
    `Biggest mover: ${w.mover ? `${w.mover.name} ${w.mover.delta >= 0 ? '+' : '-'}${Math.abs(w.mover.delta)}` : '—'}`,
    `Fines this GW: R${w.finesThisGw} · Prize pool: R${w.poolLevied}`,
    ``,
    `The Drop (R100 each):`,
    ...w.drop.map((p, i) => `  ${i + 1}. ${p.name} — ${p.pts} pts`),
    ``,
    `Top of the table:`,
    ...w.standings.map((m, i) => `  ${i + 1}. ${m.name} — ${m.pts} pts`),
    ``,
    `Full infographic attached.`,
  ].join('\n');
  return { subject: `LMP · GW${w.gw} Weekly Report`, html: emailShell(inner), text };
}

module.exports = {
  resendSend, reminderEmail, confirmationEmail, announcementEmail, sendConfirmation,
  weeklySummaryEmail, outstandingOf, auditEntry, zingerFor,
